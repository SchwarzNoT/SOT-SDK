...
void aimbot::aimbot(int vKey, float smoothX, float smoothY, Vector3 BoneAimLoc) {
    ...
    if (GetAsyncKeyState(vKey)) {
        ...
        //RotationInput
        util::Write<float>(localPlayerController + 0x418, bufferX / smoothX);
        util::Write<float>(localPlayerController + 0x41c, bufferY / smoothY);
    }
    else {
...
AFortPawn aimbot::getAimTarget() {
    ...
    for (int i = 0; i < enemyPawns.size(); i++) {
        ...
        if (!enemyPawns[i].skeleton[0][0].isZero()) {

            float distFromCrossHair = util::get2DDistance(enemyPawns[i].getW2S(), Vector3(winProperties.width / 2 + winProperties.x, winProperties.height / 2 + winProperties.y, 0));
            ...
            else if (aimPriority == 1)
            {
                ...
                if (enemyPawns[i].Distance < distBuffer) {

                    distBuffer = enemyPawns[i].Distance;
                    currTarget = i;
